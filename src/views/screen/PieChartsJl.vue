<template>
  <div>
    <div id="pieChartJl" ref="pieChartJl" class="pieChart"></div>
...
import { getPoliceAge } from "@/api/screen"
import bus from "@/bus"
...
export default {
  ...
  setup() {
    ...
    function getPoliceAgeData() {
      getPoliceAge().then(res => {
        if (res.code === 200) {
    ...
    function initChart() {
      ...
      let myPieChart = pieEchart.init(pieChartJl.value)

      myPieChart.on('click', function (params) {
        console.log(params)
        if (params.name === '0-10年') {
          retireeList(0, 10)
          title.value = '0-10年'
        } else if (params.name === '10-20年') {
          retireeList(10, 20)
          title.value = '10-20年'
