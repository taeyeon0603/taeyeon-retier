  ...
  <div class="table-container">
    <el-row class="action-bar">
      <el-col :span="24" style="display: flex; justify-content: space-between">
        <div>
          <el-button v-if="isAdmin" type="primary" icon="el-icon-plus" circle @click="handleAddBtn"></el-button>
          <el-button type="success" :disabled="selectCount != 1" icon="el-icon-edit" circle @click="handleEditBtn"> </el-button>
          <el-button v-if="isAdmin" type="danger" :disabled="selectCount == 0" icon="el-icon-delete" circle @click="handleDeleteBtn"> </el-button>
        </div>
        <div v-if="isAdmin" style="margin-right: 20px">
          ...
          <el-upload :action="uploadApi + '/policeInfo/import'" :show-file-list="false" :on-success="handleSuccess" ref="file">
            <el-button type="info" icon="el-icon-upload" circle @click="handleImportExcel"></el-button>
          </el-upload>
          <el-link icon="el-icon-download" type="danger" style="margin-left: 10px" :href="downloadApi" :underline="false">导入模板 </el-link>
        </div>
      </el-col>
    </el-row>

    <el-table v-loading="loading" ref="multipleTable" :data="tableData.data" tooltip-effect="dark" style="width: calc(100% - 20px); margin-left: 10px; margin-top: 10px" @selection-change="handleSelectChange">
      <el-table-column type="selection" width="35"> </el-table-column>

      <el-table-column type="index" label="编号" width="50"> </el-table-column>

      <el-table-column key="name" prop="name" label="姓名"> </el-table-column>
      <el-table-column key="idCard" prop="idCard" label="身份证"> </el-table-column>
      <el-table-column key="politicalIdentity" prop="politicalIdentity" label="政治面貌"> </el-table-column>

      <el-table-column key="deptName" prop="deptName" label="原部门"> </el-table-column>

      <el-table-column key="policeStation" prop="policeStation" label="居住辖区"> </el-table-column>
      <el-table-column key="retireDate" prop="retireDate" label="退休时间" :formatter="formatRetireDate"> </el-table-column>

      <el-table-column key="party" prop="party" label="退休党支部"> </el-table-column>
      <el-table-column key="score" prop="score" label="银耀分数"> </el-table-column>
      <el-table-column label="" width="100" key="operation">
        <template #default="scope">
          <el-button type="text" class="customText" size="small" @click="retireeDetail(scope.row)">详情 </el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination style="margin-top: 10px; text-align: right" background @current-change="handleCurrentChange" :current-page="tableParam.page" :page-sizes="[10, 15, 20]" :page-size="tableParam.pageSize" layout="total,prev,pager,next,jumper" :total="files_count"> </el-pagination>
...
<script>
import { reactive, ref, onMounted, inject, provide, nextTick, onBeforeUnmount, onUnmounted } from "vue"
import { postRetireList, deleteRetire, postRetireNewList, postRetireExtraList } from "@/api/retiree"
import bus from "@/bus"
...
export default {
  ...
  setup() {
    ...
    let downloadApi = process.env.VUE_APP_DOWNLOAD_URL
    ...
    function retireeList() {
      ...
      if (type === "list") {
        ...
        postRetireExtraList(tableParam)
          .then((res) => {
            // console.log(res)
            if (res.code === 200) {
              tableData.data = res.data.content
